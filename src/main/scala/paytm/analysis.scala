...
object analysis {
  def main(args: Array[String]){
    ...
    // request url split into actual url and query params
    ...
    // get time difference (< 15 minutes)
    ...
    // Get length of each session using MAX, MIN diff for each (ip,session) pair and then avg over all sessions
   ...
   // Q3. Determine unique URL visits per session. To clarify, count a hit to a unique URL only once per session.
    ...
    // Sort All users by avg session time
